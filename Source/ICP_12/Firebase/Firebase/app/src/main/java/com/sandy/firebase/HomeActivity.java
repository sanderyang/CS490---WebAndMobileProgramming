...
public class HomeActivity extends AppCompatActivity {
    ...
    private void updateUser(String theName, String thePhoneNumber) {
        // updating the user via child nodes
        if (!TextUtils.isEmpty(theName))
            mFirebaseDatabase.child(userId).child("name").setValue(theName);

        if (!TextUtils.isEmpty(thePhoneNumber))
            mFirebaseDatabase.child(userId).child("phone").setValue(thePhoneNumber);
    }
    ...
    public void userDelete(View v){
        ...
        if (theUser != null){
            theUser.delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()){
                        Toast.makeText(HomeActivity.this, "Account profile successfully deleted.", Toast.LENGTH_SHORT).show();
                    }
                    else{
    ...
    //Logs the user out of the Home Activity Screen.
